...
class OpenAIWalletAnalysisService
{
     ...
     * }
     */
    public function analyzeWalletWithFallback(Wallet $wallet): array
    {
        try {
            $result = $this->analyzeWallet($wallet);
            ...
            $this->logger->warning('Bascule vers l analyse locale du wallet.', [
            ...
            return [
                'snapshot' => $snapshot,
                'analysis' => $analysis,
                'raw_text' => json_encode($analysis, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) ?: '',
    ...
    {
        $totalTransactions = $this->countTransactions($wallet);
        $recentTransactions = $this->countRecentTransactions($wallet, 30);
        $totalCheques = $this->countCheques($wallet);
        $refusedCheques = $this->countCheques($wallet, 'refuse');
        $latestTransactions = $this->getLatestTransactions($wallet, 6);

        $frequency = $this->computeFrequencyLabel($recentTransactions);
    ...
    private function requestAnalysis(array $snapshot): array
    {
        ...
        $instructions = 'Tu es un assistant de supervision bancaire pour un back-office admin Symfony. '
            ...
            . 'risk_level doit etre une des valeurs exactes suivantes: faible, moyen, eleve. '
            . 'anomalies et recommendations doivent etre des tableaux de phrases courtes en francais. '
            . 'Reste concret, prudent et utile pour une soutenance.';
    ...
    private function normalizeAnalysis(array $decoded, array $snapshot): array
    {
        ...
        if (!in_array($riskLevel, ['faible', 'moyen', 'eleve'], true)) {
    ...
    private function inferFallbackRiskLevel(array $snapshot): string
    {
        ...
        if ((int) ($snapshot['failed_attempts'] ?? 0) >= 3) {
            $score += 1;
        }

        if ((int) ($snapshot['recent_transactions_30d'] ?? 0) >= 20) {
            $score += 1;
        }
    ...
    private function buildLocalFallbackAnalysis(array $snapshot): array
    {
        ...
        $latestTransactions = is_array($snapshot['latest_transactions'] ?? null) ? $snapshot['latest_transactions'] : [];
        ...
        if ($frequency === 'elevee' && count($latestTransactions) >= 3) {
        ...
        $summary = sprintf(
            'Le wallet affiche un solde de %.2f %s, un statut %s et une activite %s avec %d transaction(s) recente(s) sur 30 jours.',
            $balance,
            $currency,
            $status,
            $frequency,
        ...
        $explanation = match ($riskLevel) {
            'eleve' => 'Le niveau de risque est estime eleve car plusieurs signaux de vigilance sont combines, comme un blocage, des refus, des echecs ou une activite inhabituelle. Une revue admin rapide est recommandee.',
    ...
    private function countTransactions(Wallet $wallet): int
    {
    ...
    private function countRecentTransactions(Wallet $wallet, int $days): int
    {
        ...
        return (int) $this->entityManager->getRepository(Transaction::class)
            ...
            ->select('COUNT(t.idTransaction)')
            ->andWhere('t.idWallet = :walletId')
            ->andWhere('t.dateTransaction >= :since')
            ->setParameter('walletId', $wallet->getIdWallet())
    ...
    private function countCheques(Wallet $wallet, ?string $status = null): int
    {
        ...
        if ($status !== null) {
            $qb
                ->andWhere('LOWER(c.statut) = :status')
    ...
    private function getLatestTransactions(Wallet $wallet, int $limit): array
    {
        ...
        return array_map(static function (Transaction $transaction): array {
            return [
                'id' => $transaction->getIdTransaction(),
                'type' => $transaction->getType(),
                'amount' => round($transaction->getMontant(), 2),
    ...
    private function computeFrequencyLabel(int $recentTransactions): string
    {
        return match (true) {
            $recentTransactions >= 20 => 'elevee',
            $recentTransactions >= 8 => 'moderee',
            $recentTransactions >= 1 => 'faible',
            default => 'quasi nulle',
        };
    }
